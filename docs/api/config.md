...
def with_deterministic_ids(cls, id_fields: Dict[str, str]) -> "TransmogConfig":
    ...
    Args:
        id_fields: Dictionary mapping table paths to field names used for ID generation
        
    Returns:
        TransmogConfig: New configuration with deterministic ID settings
    """
...
- **id_field** (`str`, default: `"__extract_id"`): Field name for record IDs.
...
- **deterministic_id_fields** (`Dict[str, str]`, default: `{}`): Dictionary mapping table paths to field names used for deterministic ID generation.
...
- **recovery_strategy** (`str`, default: `"strict"`): Recovery strategy to use ("strict", "skip", or "partial").
...
- **EAGER**: Converts data immediately and keeps all formats in memory. Provides faster repeated access but higher memory usage.
- **LAZY**: Converts data only when needed. Intermediate conversion results aren't cached. Provides balanced memory usage.
- **MEMORY_EFFICIENT**: Minimizes memory usage by clearing intermediate data after conversion. Good for large datasets but slower for repeated conversions.

...
|------|-------------|----------|
| `EAGER` | Converts data immediately and keeps all formats in memory. Provides faster repeated access but higher memory usage. | Interactive analysis, repeated format conversions, smaller datasets |
| `LAZY` | Converts data only when needed. Intermediate conversion results aren't cached. Provides balanced memory usage. | One-time processing, moderate-sized datasets |
| `MEMORY_EFFICIENT` | Minimizes memory usage by clearing intermediate data after conversion. Good for large datasets but slower for repeated conversions. | Very large datasets, memory-constrained environments, one-time conversion |

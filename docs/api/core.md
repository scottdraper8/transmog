...
This section covers the core low-level functions in Transmog. These functions are used internally by the high-level API (`Processor` class) but can also be used directly for more control over the transformation process.
...
Processes JSON structure with parent-child relationship preservation. This is the main entry point for processing a complete JSON structure, handling both the main record and all nested arrays.

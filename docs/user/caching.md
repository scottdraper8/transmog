...
Transmog includes a value processing cache to improve performance when processing large datasets with repeated values. This document explains how to configure and use this cache system.
...
The cache implementation uses Python's `functools.lru_cache` decorator with a configurable maximum size. This provides an efficient, thread-safe caching mechanism with minimal overhead. 
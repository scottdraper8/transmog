...
def main():
    ...
    os.makedirs(os.path.join(output_dir, "streaming", "standard"), exist_ok=True)
    os.makedirs(os.path.join(output_dir, "streaming", "json"), exist_ok=True)
    os.makedirs(os.path.join(output_dir, "streaming", "csv"), exist_ok=True)
    os.makedirs(os.path.join(output_dir, "streaming", "parquet"), exist_ok=True)
    os.makedirs(os.path.join(output_dir, "streaming", "custom"), exist_ok=True)
    ...
    result.write_all_json(os.path.join(output_dir, "streaming", "standard"))
    ...
    processor.stream_process(
        ...
        entity_name="records",
        output_format="json",
        output_destination=os.path.join(output_dir, "streaming", "json"),
    ...
    json_files = [
        f
        for f in os.listdir(os.path.join(output_dir, "streaming", "json"))
        if f.endswith(".json")
    ]
    ...
    processor.stream_process(
        ...
        entity_name="records",
        output_format="csv",
        output_destination=os.path.join(output_dir, "streaming", "csv"),
    )
    ...
    csv_files = [
        f
        for f in os.listdir(os.path.join(output_dir, "streaming", "csv"))
        if f.endswith(".csv")
    ]
    ...
    try:
        ...
        processor.stream_process(
            ...
            output_destination=os.path.join(output_dir, "streaming", "parquet"),
        ...
        parquet_files = [
            f
            for f in os.listdir(os.path.join(output_dir, "streaming", "parquet"))
    ...
    custom_config = (
        ...
        .with_naming(
            ...
            deeply_nested_threshold=3,  # Handle deeply nested fields
        )
        .with_processing(
            cast_to_string=False,  # Keep original data types
            skip_null=False,  # Include null values
            processing_mode=tm.ProcessingMode.LOW_MEMORY,  # Memory-optimized mode
        )
        .with_metadata(
    ...
    processor.stream_process(
        ...
        entity_name="records",
        output_format="json",
        output_destination=os.path.join(output_dir, "streaming", "custom"),
    ...
    custom_files = [
        f
        for f in os.listdir(os.path.join(output_dir, "streaming", "custom"))
        if f.endswith(".json")
    ]

...
def main():
    ...
    output_dir = os.path.join(
        os.path.dirname(__file__), "..", "data", "output", "csv_processing"
    )
    os.makedirs(output_dir, exist_ok=True)

    # Create sample CSV file
    csv_filepath = os.path.join(output_dir, "sample_employees.csv")
    ...
    csv_result = processor.process_file(
        file_path=csv_filepath, entity_name="employees", file_format="csv"
    )
    ...
    # Create custom configuration for CSV processing
    csv_config = tm.TransmogConfig.default().with_processing(
        ...
        cast_from_string=True,  # Try to cast values to appropriate types
    ...
    # Create processor that treats underscore as a nested delimiter
    nested_config = (
        tm.TransmogConfig.default()
        .with_naming(
            interpret_separator="_"  # Interpret underscore as nesting in column names
        )
        .with_processing(
            cast_from_string=True,  # Try to cast values to appropriate types
    ...
    # Create configuration that splits comma-separated values into arrays
    array_config = tm.TransmogConfig.default().with_processing(
        csv_array_delimiter=",",  # Split comma-separated values into arrays
        cast_to_string=False,  # Keep original types
    )
    ...
    json_files = csv_result.write_all_json(base_path=json_dir, indent=2)
    print(f"Wrote {len(json_files)} JSON files to {json_dir}")

    # Write to CSV (re-export)
    csv_dir = os.path.join(output_dir, "csv_export")
    csv_files = csv_result.write_all_csv(base_path=csv_dir)
    ...
    try:
        ...
        parquet_files = csv_result.write_all_parquet(base_path=parquet_dir)

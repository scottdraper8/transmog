...
from transmog import AggregateOperation, GroupBy, Processor


def main():
    """Main function to demonstrate data aggregation."""
    ...
    for record in region_sales.records:
        ...
        print(f"  {record['region']}: {record['total_units']} units, ${revenue:.2f}")

    print("\nSales by Product (with statistics):")
    for record in product_stats.records:
        avg_revenue = record["avg_revenue"]
        print(
            f"  {record['product']}: {record['total_units']} units, "
            f"${avg_revenue:.2f} avg"
        )
    ...
    for record in monthly_sales.records:
        ...
        print(
            f"  {record['date']}: {record['monthly_units']} units, "

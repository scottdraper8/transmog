...
def main():
    ...
    print(
        "EAGER mode converts data immediately and caches results for faster repeated access."
    )
    ...
    print(
        f"  Speed improvement: {eager_first['time'] / max(eager_second['time'], 0.0001):.2f}x faster"
    )
    ...
    print("\nMemory Usage Comparison:")
    print(
        f"  EAGER mode:           {eager_first['memory_delta'] + eager_csv['memory_delta']:.2f} MB"
    )
    print(
        f"  LAZY mode:            {lazy_first['memory_delta'] + lazy_csv['memory_delta']:.2f} MB"
    )
    print(
        f"  MEMORY_EFFICIENT mode: {efficient_first['memory_delta'] + file_write_metrics['memory_delta']:.2f} MB"
    )

    # Speed comparison for repeated operations
    print("\nSpeed Comparison for Repeated Operations:")
    print(
        f"  EAGER mode:           {eager_second['time']:.4f} seconds (fastest for repeated access)"
    )
    ...
    if dataset_size < 10_000:
        # Small dataset - use eager mode (default)
        result = processor.process(data, entity_name="records")

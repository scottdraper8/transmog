"""Tests for flatten(), flatten_file(), and flatten_stream() API functions."""
...
class TestFlattenConfiguration:
    ...
    def test_flatten_with_dot_separator(self):
        """Test flattening with dot separator."""
        data = {"level1": {"level2": {"value": "test"}}}
        config = TransmogConfig(separator=".")
        result = tm.flatten(data, name="test", config=config)

        assert len(result.main) == 1
        assert "level1.level2.value" in result.main[0]
        assert result.main[0]["level1.level2.value"] == "test"
...
class TestFlattenFile:
    ...
    def test_flatten_file_auto_name(self, json_file):
        ...
        result = tm.flatten_file(json_file)
        assert_valid_result(result)

    def test_flatten_file_with_config(self, json_file):
        """Test flatten_file with configuration options."""
...
class TestParameterValidation:
    ...
    def test_invalid_separator_parameter(self):
        """Test flatten with invalid separator."""
        data = {"test": "data"}
    ...
    def test_invalid_boolean_parameters(self):
        """Test flatten with boolean parameters."""
        data = {"test": "data"}

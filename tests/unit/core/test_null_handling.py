...
from transmog.types import NullHandling


class TestNullHandlingSkip:
    """Test NullHandling.SKIP mode (default)."""
...
class TestNullHandlingInclude:
    """Test NullHandling.INCLUDE mode."""

    def test_include_null_values(self):
        """Test that null values are included when configured."""
        data = {"id": 1, "name": "test", "value": None, "description": "valid"}

        config = TransmogConfig(null_handling=NullHandling.INCLUDE)
...
class TestNullHandlingConsistency:
    ...
    def test_consistent_behavior_across_batches(self):
        ...
        config = TransmogConfig(null_handling=NullHandling.SKIP)
    ...
    def test_null_handling_with_different_separators(self):
        """Test null handling works with different separators."""
        data = {"nested": {"null_field": None, "valid": "data"}}

        config_underscore = TransmogConfig(
            separator="_", null_handling=NullHandling.SKIP
        )
        config_dot = TransmogConfig(separator=".", null_handling=NullHandling.SKIP)
        ...
        # Both should skip null field
        assert "nested_null_field" not in result_underscore.main[0]
        assert "nested.null_field" not in result_dot.main[0]
